// src/pages/InstructorAttendancePage/InstructorAttendancePage.jsx
//
// Instructor-scoped attendance page.
// Instructors can: create, update attendance for their own lessons.
// Instructors CANNOT: delete records (backend blocks it via get_permissions).
//
// APIs used:
//   GET    /api/attendance/                          → list (role-filtered by backend)
...
//   GET    /api/lesson/                              → instructor's own lessons
//   GET    /api/studentprofile/                      → students in instructor's school
...
/* ─── API layer ──────────────────────────────────────────────── */
const attendanceApi = {
  // List — backend filters to instructor's own lessons automatically
  getAll: (qs = '') =>
    axios.get(`${API}/attendance/${qs ? `?${qs}` : ''}`, {
      headers: authHeader(),
    }).then(r => r.data),
  ...
  // Update a record (presence, hours_completed, notes)
  update: (id, data) =>
    axios.patch(`${API}/attendance/${id}/`, data, {
      headers: authHeader(),
    }).then(r => r.data),

  // Bulk create — up to 100 records at once
  bulkCreate: (records) =>
    axios.post(`${API}/attendance/bulk_create/`, { attendance_records: records }, {
      headers: authHeader(),
    }).then(r => r.data),

  // Lesson attendance summary (drawer)
  lessonSummary: (lessonId) =>
    axios.get(`${API}/attendance/lesson_summary/?lesson_id=${lessonId}`, {
      headers: authHeader(),
    }).then(r => r.data),
  ...
  // Instructor's own lessons only (backend filters by role)
  getLessons: () =>
    axios.get(`${API}/lesson/`, {
      headers: authHeader(),
    }).then(r => r.data.results ?? r.data),

  // Students in instructor's school
  ...
  <tr><td colSpan={7}>
    <div className="flex flex-col items-center justify-center py-20 gap-5">
      ...
      <div className="text-center">
        ...
        <p className="text-[11px] text-white/25 mt-1">Start by marking attendance for one of your lessons.</p>
      </div>
...
// On CREATE: student + lesson are required and locked after submit.
// On EDIT:   student + lesson are read-only (disabled) — only presence,
//            hours_completed, and notes are editable.
const EMPTY_FORM = {
  student: '', lesson: '', presence: true, hours_completed: '', notes: '',
};
...
const AttendanceModal = ({ open, onClose, onSuccess, record, lessons, students }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      if (isEdit && record) {
        setForm({
          // student + lesson are read-only on edit — kept in state for display only
  ...
  const validate = () => {
    const e = {};
    if (!isEdit && !form.student)  e.student = 'Select a student';
    if (!isEdit && !form.lesson)   e.lesson  = 'Select a lesson';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        // Only send editable fields — never student/lesson on update
        const payload = {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
      ...
      <div className="bg-[#0B1221] border border-white/[0.08] rounded-2xl w-full max-w-[500px]
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/30">
                {isEdit ? 'Update presence, hours, and notes' : 'Record a student attendance'}
              </p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-4">
          ...
          {/* Student + Lesson — disabled on edit */}
          <div className="grid grid-cols-2 gap-3">
            <Field label="Student" error={errors.student}>
              <select className={cls(selectCls, isEdit && 'opacity-50 cursor-not-allowed')}
                value={form.student}
                ...
                {students.map(s => (
                  <option key={s.id} value={s.id}>
                    {[s.user_first_name, s.user_last_name].filter(Boolean).join(' ') || s.user_username || `Student #${s.id}`}
                  </option>
                ))}
              </select>
              {isEdit && <p className="text-[9px] text-white/20 mt-0.5">Cannot change after creation</p>}
            </Field>
          ...
          <Field label="Hours Completed" error={errors.hours_completed}>
            <input
              type="number" min={0} max={480} step={0.5}
              className={cls(inputCls, !form.presence && 'opacity-40')}
              placeholder="e.g. 2"
      ...
      <div className="fixed right-0 top-0 h-full w-[360px] z-[55]
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          <div className="bg-blue-600/08 border border-blue-500/15 rounded-2xl px-4 py-4">
            <p className="font-sora text-[15px] font-bold text-white mb-1">{lesson?.title || '—'}</p>
          ...
          <div className="flex flex-col gap-1.5">
            <p className="text-[9px] font-bold text-white/25 uppercase tracking-[0.6px]">Attendance Records</p>
            {records.length === 0
...
/* ─── Attendance Row — NO delete button for instructors ──────── */
const AttendanceRow = ({ record, onEdit, onLessonClick }) => (
  <tr className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors group cursor-default">
    ...
    {/* Lesson — clickable → opens summary drawer */}
    <td className="px-4 py-3.5">
      <button
    ...
    <td className="px-4 py-3.5">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[10px] text-white/50">{record.hours_completed || 0}h</span>
      </div>
    </td>
    ...
    {/* Edit only — no delete (backend blocks it for instructors) */}
    <td className="px-4 py-3.5">
...
const InstructorAttendancePage = () => {
  ...
  /* ── fetch lessons + students + stats on mount ── */
  useEffect(() => {
    // Lessons: backend returns only instructor's own lessons (role-filtered)
    attendanceApi.getLessons()
      .then(data => setLessons(Array.isArray(data) ? data : data?.results || []))
      .catch(() => {});

    // Stats: backend returns instructor-specific stats
    attendanceApi.statistics()
      .then(setStats)
      .catch(() => {});
  }, []);

  // Load students once we have lessons (need a school id)
  // We derive school from the first lesson returned
  useEffect(() => {
    if (lessons.length === 0) return;
    const schoolId = lessons[0]?.school;
    if (!schoolId) return;
    attendanceApi.getStudents(schoolId)
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      // Backend filterset uses 'presence' as a boolean field
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="h-[52px] flex-shrink-0 bg-[#0B1221] border-b border-white/[0.06]
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[11px] text-white/30">Track student attendance for your lessons</span>
          </div>
          <div className="flex-1" />
          <button onClick={openCreate}
            className="flex items-center gap-1.5 px-3.5 py-1.5 bg-emerald-600
              hover:bg-emerald-500 rounded-[7px] text-[11px] font-semibold text-white transition-all">
            {Icon.plus} Mark Attendance
          </button>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          ...
          {/* Stats — uses backend stats (instructor-scoped) with local fallback */}
          <div className="grid grid-cols-4 gap-3">
            <StatCard emoji="📋" label="Total records"
              value={stats?.total_attendance_records ?? totalCount} accent="blue" />
            <StatCard emoji="✅" label="Present"
              value={stats?.students_present ?? presentCount} accent="emerald" />
            <StatCard emoji="❌" label="Absent"
              value={stats?.students_absent ?? absentCount} accent="red" />
            <StatCard emoji="📊" label="Attendance rate"
              value={stats?.overall_attendance_rate != null
          ...
          <div className="bg-[#0F1A2E] border border-white/[0.07] rounded-[14px] flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-5 py-3.5 border-b border-white/[0.06] flex-wrap">
              ...
              <div className="flex bg-white/[0.04] rounded-[8px] p-0.5 gap-0.5">
                {[['', 'All'], ['true', '✓ Present'], ['false', '✗ Absent']].map(([v, l]) => (
                  <button key={v} onClick={() => setPresenceFilter(v)}
                    className={cls(
                      'px-3 py-1.5 rounded-[6px] text-[10px] font-semibold transition-all',
                      presenceFilter === v ? 'bg-white/[0.1] text-white' : 'text-white/30 hover:text-white/60',
                    )}>
                    {l}
                  </button>
                ))}
