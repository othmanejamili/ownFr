...
export const useCommunicationTemplates = (initialParams = {}) => {
  const [params, setParams]         = useState(initialParams);
  const [search, setSearch]         = useState('');
  const [typeFilter, setTypeFilter] = useState('');

  const { data, loading, error, execute } = useAsync(
    () => communicationTemplateAPI.list(params),
    true,
    [JSON.stringify(params)],
  );
  ...
  const templates = all.filter((t) => {
    ...
    const matchType = !typeFilter || t.template_type === typeFilter;
...
export const useTemplateMutations = (onSuccess) => {
  const [saving, setSaving]     = useState(false);
  const [mutError, setMutError] = useState(null);

  const wrap = (fn) => async (...args) => {
    setSaving(true); setMutError(null);
    try { const r = await fn(...args); return r; }
    catch (err) { setMutError(err?.response?.data ?? err.message ?? 'Error'); throw err; }
    finally { setSaving(false); }
  };

  const createTemplate    = wrap(async (data)        => { const r = await communicationTemplateAPI.create(data);             onSuccess?.('created',    r); return r; });
  const updateTemplate    = wrap(async (id, data)    => { const r = await communicationTemplateAPI.partialUpdate(id, data);  onSuccess?.('updated',    r); return r; });
  const deleteTemplate    = wrap(async (id)          => { await communicationTemplateAPI.destroy(id);                        onSuccess?.('deleted',    { id }); });
  const duplicateTemplate = wrap(async (id, newName) => { const r = await communicationTemplateAPI.duplicate(id, { new_name: newName }); onSuccess?.('duplicated', r); return r; });
  const toggleActive      = wrap(async (id)          => { const r = await communicationTemplateAPI.toggleActive(id);         onSuccess?.('toggled',    r); return r; });
  const previewTemplate   = wrap(async (id, sid)     => { const r = await communicationTemplateAPI.preview(id, sid ? { student_id: sid } : {}); return r; });

  return {
    saving, mutError, setMutError,
    createTemplate, updateTemplate, deleteTemplate,
    duplicateTemplate, toggleActive, previewTemplate,
  };
};

export const useTemplateUsageStats = () => {
  const { data, loading, error, execute } = useAsync(communicationTemplateAPI.usageStats);
  return { stats: data, loading, error, refresh: execute };
};

export const useAvailableVariables = () => {
  const { data, loading, error } = useAsync(communicationTemplateAPI.availableVariables);
  return { variables: data?.variables ?? {}, loading, error };
};

export const useTemplatesByType = (params = {}) => {
  const { data, loading, error, execute } = useAsync(
    () => communicationTemplateAPI.byType(params),
    true,
    [JSON.stringify(params)],
  );
  return { grouped: data?.grouped_templates ?? {}, rawData: data, loading, error, refresh: execute };
};