// src/pages/Dashboard/Owner/Template/CommunicationTemplatePage.jsx
    ...
    <div className={`bg-[#0B1221] border rounded-2xl overflow-hidden transition-all duration-200 hover:border-white/[0.12] group
      ...
      <div className="px-4 pb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <ActionBtn label="Preview"   onClick={() => onPreview(template)}  color="blue"   />
          <ActionBtn label="Edit"      onClick={() => onEdit(template)}     color="blue"   />
          <ActionBtn label="Duplicate" onClick={() => onDuplicate(template)}color="violet" />
          <ActionBtn label={template.is_active ? 'Deactivate' : 'Activate'} onClick={() => onToggle(template.id)} color="amber" />
          <ActionBtn label="Delete"    onClick={() => onDelete(template.id)}color="red"    />
        </div>
      </div>
    </div>
  );
...
const CommunicationTemplatePage = () => {
  const { user } = useAuth();
  ...
  const handleEdit  = (t) => { setSaveError(null); setFormModal({ open: true, initial: t }); };
  const handleNew   = ()  => { setSaveError(null); setFormModal({ open: true, initial: null }); };
  ...
  const handleDelete = async (id) => {
    ...
    try { await mutations.deleteTemplate(id); push('✓ Template deleted'); refresh(); }
    catch (err) {
      const msg = err?.response?.data?.detail ?? err?.response?.data ?? 'Delete failed';
      push(typeof msg === 'string' ? msg : JSON.stringify(msg), 'error');
    }
  };

  const handleDuplicate = async (t) => {
    const name = window.prompt('New template name:', `Copy of ${t.name}`);
    if (!name) return;
    try { await mutations.duplicateTemplate(t.id, name); }
    catch { push('Duplicate failed', 'error'); }
  };

  const handleToggle = async (id) => {
    try { await mutations.toggleActive(id); }
    catch { push('Toggle failed', 'error'); }
  };
