// Pricing.jsx — PLANS now come from /api/subscriptionplan/
// Every other component (PlanCard, ComparisonTable, FAQ, etc.) is UNCHANGED.
// Only changes are marked with ── API ──
...
const BillingToggle = ({ billing, onChange }) => {
  ...
  return (
    <div className="inline-flex items-center gap-3.5 bg-white/[0.04] border border-white/[0.08]
      ...
      <button onClick={() => onChange(isAnnual ? 'monthly' : 'annual')}
...
// ── Accent config (unchanged) ─────────────────────────────────
const ACCENT = {
  blue: {
    iconBg: 'bg-blue-600/18 text-blue-400', nameTxt: 'text-blue-400',
  ...
  purple: {
    iconBg: 'bg-violet-600/20 text-violet-400', nameTxt: 'text-violet-400',
  ...
  cyan: {
    iconBg: 'bg-cyan-600/16 text-cyan-400', nameTxt: 'text-cyan-400',
...
// ── Plan card — ONE CHANGE: icon now uses dangerouslySetInnerHTML ──
const PlanCard = ({ plan, billing }) => {
  const a = ACCENT[plan.accent] ?? ACCENT.blue;
  ...
  return (
    <div className={[
      ...
      {/* ── API: icon rendered from iconPath string ── */}
      <div className={`relative z-10 w-11 h-11 rounded-xl flex items-center justify-center
        ...
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
          dangerouslySetInnerHTML={{ __html: plan.iconPath }} />
      </div>
...
const PlanSkeleton = () => (
  <div className="bg-[#0F1A2E] border border-white/[0.07] rounded-[22px] p-7 animate-pulse">
    ...
    <div className="h-10 w-full bg-white/5 rounded-xl mb-6" />
    <div className="h-px bg-white/5 mb-5" />
    {[1,2,3,4,5,6].map(i => (
      <div key={i} className="flex gap-2.5 mb-2.5">
        <div className="w-[17px] h-[17px] rounded bg-white/5 flex-shrink-0" />
        <div className="h-[17px] bg-white/5 rounded flex-1" />
      </div>
    ))}
  </div>
);
...
// ── Error state ───────────────────────────────────────────────
const PlansError = ({ message, onRetry }) => (
  <div className="col-span-3 flex flex-col items-center justify-center py-20 gap-4">
    <div className="w-12 h-12 rounded-2xl bg-red-500/10 border border-red-500/25 flex items-center justify-center">
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <path d="M10 2L18 17H2L10 2Z" stroke="#F87171" strokeWidth="1.4"/>
        <path d="M10 8v4M10 14.5v.5" stroke="#F87171" strokeWidth="1.4" strokeLinecap="round"/>
      </svg>
    </div>
    <p className="text-white/40 text-[14px] font-dm text-center max-w-xs">{message}</p>
    <button onClick={onRetry}
      className="px-5 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl
        text-white/60 hover:text-white text-[13px] font-dm transition-all">
      Try again
    </button>
  </div>
);
...
const TABLE_ROWS = [
  ...
  { cat: 'SCHEDULING' },
  { label: 'Smart calendar',          starter: '✓', pro: '✓', enterprise: '✓' },
  { label: 'Conflict detection',      starter: '✓', pro: '✓', enterprise: '✓' },
  { label: 'AI scheduling optimizer', starter: '—', pro: '—', enterprise: '✓' },
  { cat: 'COMMUNICATIONS' },
  { label: 'Email reminders',          starter: '✓', pro: '✓', enterprise: '✓' },
  { label: 'SMS reminders',            starter: '—', pro: '✓', enterprise: '✓' },
  { label: 'Custom message templates', starter: '—', pro: '✓', enterprise: '✓' },
  { cat: 'PAYMENTS & BILLING' },
  { label: 'Manual invoicing', starter: '✓', pro: '✓', enterprise: '✓' },
  { label: 'Auto invoicing',   starter: '—', pro: '✓', enterprise: '✓' },
  { label: 'Payment plans',    starter: '—', pro: '✓', enterprise: '✓' },
  { cat: 'ANALYTICS & REPORTS' },
  { label: 'Basic reports',            starter: '✓', pro: '✓', enterprise: '✓' },
  { label: 'Advanced analytics',       starter: '—', pro: '✓', enterprise: '✓' },
  { label: 'Custom reports & exports', starter: '—', pro: '—', enterprise: '✓' },
  { cat: 'PLATFORM & SUPPORT' },
  { label: 'API access',    starter: '—',     pro: '—',              enterprise: '✓' },
  { label: 'White-label',   starter: '—',     pro: '—',              enterprise: '✓' },
  { label: 'Support level', starter: 'Email', pro: 'Priority chat',  enterprise: '24/7 phone' },
  { label: 'Uptime SLA',    starter: '—',     pro: '—',              enterprise: '99.9%' },
];

const ComparisonTable = () => (
  <section className="max-w-4xl mx-auto px-11 pb-20">
    <div className="text-center mb-10">
      <h2 className="font-sora text-[32px] font-black text-white tracking-[-1px] mb-3">Compare all features</h2>
    ...
    <div className="bg-[#0F1A2E] border border-white/[0.08] rounded-[18px] overflow-hidden">
      <div className="grid grid-cols-4 bg-[#162035] border-b border-white/[0.07]">
        <div className="px-5 py-4 text-[11px] font-semibold text-white/40 tracking-[0.6px] font-dm">FEATURE</div>
        {['Starter','Pro ✦','Enterprise'].map((h, i) => (
          <div key={h} className={`px-5 py-4 text-center text-[12px] font-bold tracking-[0.5px] font-dm
            ${i === 1 ? 'text-violet-400' : 'text-white/40'}`}>{h}</div>
        ))}
      </div>
      {TABLE_ROWS.map((row, i) => {
        if (row.cat) return (
          <div key={i} className="grid grid-cols-4 bg-white/[0.02] border-b border-white/[0.05]">
            <div className="col-span-4 px-5 py-2.5 text-[10px] font-bold text-white/25 tracking-[0.8px] font-dm">{row.cat}</div>
          </div>
        );
        return (
          <div key={i} className="grid grid-cols-4 border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors duration-150 group">
            <div className="px-5 py-3.5 text-[13px] text-white/55 font-dm">{row.label}</div>
            <div className="px-5 py-3.5 text-[12px] text-white/55 text-center font-semibold font-dm">{row.starter}</div>
            <div className="px-5 py-3.5 text-[12px] text-white/65 text-center font-semibold font-dm bg-violet-600/[0.06] group-hover:bg-violet-600/[0.1]">{row.pro}</div>
            <div className="px-5 py-3.5 text-[12px] text-white/55 text-center font-semibold font-dm">{row.enterprise}</div>
          </div>
        );
      })}
    </div>
...
const FaqItem = ({ item, open, onToggle }) => (
  <div className={`border rounded-xl overflow-hidden transition-all duration-200 mb-2 bg-[#0F1A2E]
    ${open ? 'border-violet-700/50' : 'border-white/[0.07] hover:border-white/[0.12]'}`}>
    ...
    <div className={`overflow-hidden transition-all duration-350 ease-in-out ${open ? 'max-h-48 pb-5' : 'max-h-0'}`}>
...
const FaqSection = () => {
  ...
  return (
    <section className="max-w-2xl mx-auto px-11 pb-20">
      <div className="text-center mb-10">
        <h2 className="font-sora text-[30px] font-black text-white tracking-[-0.8px] mb-3">Questions? Answered.</h2>
        <p className="text-[14px] text-white/40 font-dm">Everything you need to know before signing up.</p>
      </div>
...
const SocialProof = () => (
  <div className="flex items-center justify-center gap-8 py-8 px-11 border-t border-b border-white/[0.06] bg-white/[0.015] mb-16 flex-wrap">
    {[{ val:'248+',lbl:'Schools active' },{ val:'4.9',lbl:'★ average rating' },{ val:'14d',lbl:'Free trial, no card' },{ val:'91%',lbl:'Avg pass rate' }].map((s) => (
      <div key={s.val} className="text-center px-6 border-r border-white/[0.06] last:border-0">
        <div className="font-sora text-[26px] font-black text-white tracking-[-1px] leading-none mb-1">{s.val}</div>
...
const CtaStrip = () => (
  <div className="mx-11 mb-16 relative rounded-[22px] overflow-hidden border border-violet-700/25 bg-gradient-to-br from-[#0F1A2E] via-[#13173a] to-[#0F1A2E]">
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
      <div className="w-96 h-96 rounded-full" style={{ background: 'radial-gradient(circle, rgba(124,58,237,0.13) 0%, transparent 70%)' }} />
    </div>
    <div className="relative z-10 text-center px-12 py-16">
      ...
      <p className="text-[15px] text-white/40 mb-10 leading-relaxed font-dm max-w-lg mx-auto">
        Join 248+ driving schools already saving hours every week.<br />No card required, full access for 14 days.
      </p>
      <div className="flex gap-3 justify-center items-center flex-wrap mb-8">
        <button className="px-8 py-3.5 bg-violet-700 hover:bg-violet-600 text-white text-[14px]
          ...
          shadow-[0_8px_24px_rgba(124,58,237,0.25)] font-dm">
          Start free with Pro
        </button>
        <button className="flex items-center gap-2 px-6 py-3.5 border border-white/15 rounded-xl
          ...
          hover:text-white transition-all duration-200 font-dm">
      ...
      <div className="flex items-center justify-center gap-4 flex-wrap">
        <TrustItem text="No credit card" /><TrustSep />
        <TrustItem text="Cancel anytime" /><TrustSep />
        <TrustItem text="Setup in 5 minutes" /><TrustSep />
...
// ─────────────────────────────────────────────────────────────
// ROOT — PLANS grid now uses real API data
// ─────────────────────────────────────────────────────────────
const Pricing = () => {
  const [billing, setBilling] = useState('monthly');

  // ── API: fetch plans from backend ──
  ...
  return (
    <div className="min-h-screen bg-[#060B18] font-dm text-white overflow-x-hidden">
      ...
      {/* ── API: Plans grid — skeleton / error / real data ── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto px-11 mb-20">
        {loading && [1, 2, 3].map(i => <PlanSkeleton key={i} />)}
        {!loading && error && <PlansError message={error} onRetry={() => window.location.reload()} />}
